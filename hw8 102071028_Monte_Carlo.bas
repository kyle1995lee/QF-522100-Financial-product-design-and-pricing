...
Function BinOptionValueTest(imod, iopt, iea, S, X, r, q, tyr, sigma, nstep, cr, f, ct, pt)
...
'   Choice between imod=0 (JR), imod=1 (Cox,Ross&Rubinstein) and imod=2 (Leisen&Reimer)
    ...
    For j = nstep - 1 To 0 Step -1
        ...
        If iea = 2 Then
            If j > nstep / 2 Then
                vvec(i) = Application.Max(Application.Min(vvec(i), ct), (S * (u ^ i) * (d ^ (j - i))) * cr)
            Else
                vvec(i) = Application.Max(vvec(i), iopt * (S * (u ^ i) * (d ^ (j - i))) * cr)
            End If
            
            If j = nstep / 2 Then
                vvec(i) = Application.Max(vvec(i), pt)
            Else
                vvec(i) = Application.Max(vvec(i), iopt * (S * (u ^ i) * (d ^ (j - i))) * cr)
            End If
